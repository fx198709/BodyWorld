...
#define DeviceIDIsNull(a) ((a)==nil || (a)==NULL || (NSNull *)(a)==[NSNull null] || [(NSString *)a length] == 0)
...
+ (NSString *)getIDFAString{
    //获取IDFA
    __block NSString *resultIDFA = @"";
    if (@available(iOS 14.0, *)) {
        [ATTrackingManager requestTrackingAuthorizationWithCompletionHandler:^(ATTrackingManagerAuthorizationStatus status) {
            if (status == ATTrackingManagerAuthorizationStatusAuthorized) {
                resultIDFA = [[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString];
            }else {
    ...
    //判断IDFA是否为空
//    resultIDFA = [[resultIDFA stringByReplacingOccurrencesOfString:@"-" withString:@""] stringByReplacingOccurrencesOfString:@"0" withString:@""];
    if (resultIDFA.length <1) {
        resultIDFA = @"";
    }
